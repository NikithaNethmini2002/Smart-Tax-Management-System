...
const Register = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    try {
      // Format birthday to simple YYYY-MM-DD format instead of ISO string
      const formattedValues = {
        ...values,
      };
      
      console.log('Submitting registration with:', formattedValues);
      const response = await register(formattedValues);
      
      // Store validation errors if they exist
      if (response.errors) {
            ...
            <Alert severity="error" sx={{ width: '100%', mb: 2 }}>
              ...
              <ul>
                {validationErrors.map((err, index) => (
                  <li key={index}>{err.msg} (field: {err.path})</li>
                ))}
              </ul>
            </Alert>
          )}
